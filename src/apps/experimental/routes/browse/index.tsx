...
/** Tab indices for view-backed browse modes. Matches the tab mappings in movies/index.tsx and shows/index.tsx. */
const TAB_INDEX: Record<string, Record<string, number>> = {
    [CollectionType.Movies]: {
        [LibraryTab.Genres]: 4,
        [LibraryTab.Studios]: 5,
        [LibraryTab.Favorites]: 2,
        [LibraryTab.Trending]: 6,
        [LibraryTab.TopRated]: 7
    },
    [CollectionType.Tvshows]: {
        [LibraryTab.Genres]: 3,
        [LibraryTab.Networks]: 4,
        [LibraryTab.Favorites]: 6,
        [LibraryTab.Trending]: 7,
        [LibraryTab.TopRated]: 8
    }
};

/** Route paths for each collection type. */
const COLLECTION_PATH: Partial<Record<CollectionType, string>> = {
...
const Browse: FC = () => {
    ...
    const libraryPath = collectionType ? COLLECTION_PATH[collectionType] : undefined;
    ...
    const onModeClick = useCallback((definition: BrowseModeDefinition) => {
        ...
        if (definition.view) {
            const index = collectionType ? TAB_INDEX[collectionType]?.[definition.view] : undefined;
